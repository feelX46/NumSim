...
void write VTKMasterfile(const MultiIndexType& griddimension, const MultiIndexType& processorgridcoords, const MultiIndexType& localgriddimension){

	  IndexType iMax = griddimension[0] - 1;
	  IndexType jMax = griddimension[1] - 1;
	  int stencilwidth =1;         //Todo: kp was das ist!!!
	  double x1 = processorgridcoords[0]*(localgriddimension[0]-stencilwidth);
	  double x2 = (porocessorgridcoords[0]+1)*(localgriddimension[0])+stencilwidth -1;
	  double x3 = processorgridcoords[1]*(localgriddimension[1]) - stencilwidth;
	  double x4 = (processorgridcoords[1]+1)*(localgriddimension[1]+stencilwidth -1);

	  std::string filename;
	  filename.append ("./");
	  filename.append (output);
	  filename.append ("/");
	  filename.append ("solution.pvtr");
	  ...
	  os << "<?xml version=\"1.0\"?>" << std::endl
	    ...
	    << "0" << " " << (jMax - 1) << " "
	    << "0" << " " << "0" << " "
	    << "\" GhostLevel=\"" << stencilwidth << "\">" << std::endl
	    ...
	    <<"\" Source=\"solution_processor_myRank.vtr\"/>"<<std::endl
	    <<"<PPointData>"<<std::endl
...
void wirte VTKSlavefile(){
	IndexType iMax = griddimension[0] - 1;
	IndexType jMax = griddimension[1] - 1;
	int stencilwidth=1; //Todo: was ist das?
	x5=processorgridcoords[0]*localgriddimension[0]-stencilwidth;
	x6=processorgridcoords[1]*localgriddimension[1]-stencilwidth;
	x7=(processorgridcoords[d]+1)*localgriddimension[d]+stencilwidth;
	x8=(processorgridcoords[d]+1)*localgriddimension[d]+stencilwidth;
